...
Open verification item (do this empirically once running, not via more ILSpy)
IImageEncoder.CreateImageCollage's actual behavior when given fewer than 4 Images was not inspected (we only saw the ImageProcessor wrapper, not the concrete encoder implementation). The plan above defensively pads to 4 by duplication in our own code regardless, so this is not a blocker — but worth a quick real-world test with e.g. 2 real posters to confirm whether our manual padding is even necessary, or whether the encoder already handles it (in which case the padding loop is just harmless redundancy, safe to leave in either way).